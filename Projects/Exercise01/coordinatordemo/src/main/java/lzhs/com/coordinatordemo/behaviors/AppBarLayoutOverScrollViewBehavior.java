...
public class AppBarLayoutOverScrollViewBehavior extends AppBarLayout.Behavior {
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
        boolean headler = super.onLayoutChild(parent, abl, layoutDirection);
        if (mToolBar == null) mToolBar = parent.findViewWithTag(TAG_TOOLBAR);
        if (middleLayout == null) middleLayout = parent.findViewWithTag(TAG_MIDDLE);
        if (mTargetView == null) {
            mTargetView = parent.findViewWithTag(TAG_OVERSCROLL);
            if (mTargetView != null) initial(abl);
            abl.addOnOffsetChangedListener(new AppBarLayout.OnOffsetChangedListener() {
                @Override
                public final void onOffsetChanged(AppBarLayout appBarLayout, int i) {
                    float alpha = Float.valueOf(Math.abs(i)) / Float.valueOf(appBarLayout.getTotalScrollRange());
                    System.out.println("int i = " + i + ";  appBarLayout.getTotalScrollRange() = "
